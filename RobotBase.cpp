...
RobotBase::RobotBase(int move_in, int armor_in, WeaponType weapon_in)
    ...
    m_grenades=0;
    if(weapon_in==grenade){m_grenades=15;}
    // Validate move input
    if(move_in<2){m_move=2;}
    else if(move_in>5){m_move=5;}
    else{m_move=move_in;}
    // Calculate maximum armor based on the move value
    int max_armor=7-m_move;
    // Validate armor input
    if(armor_in<0){m_armor=0;}
    else if(armor_in>max_armor){m_armor=max_armor;}
    else{m_armor=armor_in;}
    // blank out location
    m_location_row=0;
    m_location_col=0;}
...
int RobotBase::get_health(){return m_health;}
// Get the robot's armor level
int RobotBase::get_armor(){return m_armor;}
// Get the robot's movement range
int RobotBase::get_move_speed(){return m_move;}
// Get the robot's weapon type
WeaponType RobotBase::get_weapon(){return m_weapon;}
int RobotBase::get_grenades(){return m_grenades;}
void RobotBase::decrement_grenades(){m_grenades--;
    if(m_grenades<0)m_grenades=0;}
...
void RobotBase::disable_movement(){m_move=0;}
...
std::string RobotBase::print_stats()const{
    ...
    return stats.str();}
// Destructor
RobotBase::~RobotBase(){/* No additional cleanup required*/}
